...
pub(crate) fn run(dry_run: bool, since: Option<&str>) -> Result<()> {
    let ctx = CommandContext::open(None)?;
    let repo = ctx.session.repo();

    // Manual pruning is the date-based one: the window comes from --since, or
    // from a project default if one is configured. Auto-prune no longer uses a
    // window at all — it works to key and size limits.
    let since = if let Some(since) = since {
    ...
    } else {
        {
            let Some(entry) = ctx
                ...
                eprintln!("  git meta prune --since 6m");
